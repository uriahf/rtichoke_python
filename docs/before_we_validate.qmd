...
```{python}
from rtichoke.helpers.sandbox_observable_helpers import (
  assign_and_explode_polars, _excluded_events_df, _aj_adjusted_events)
...
fixed_time_horizons = [10.0, 30.0, 50.0]
...
def _long_excluded_for_combo(exploded: pl.DataFrame, ca: str, co: str) -> pl.DataFrame:
    return (
        _excluded_events_df(exploded, ca, co)
...
excluded_frames = [
    _long_excluded_for_combo(exploded, ca, co)
    for ca in censoring_assumptions
    for co in competing_assumptions
]
...
Plot.plot({
  ...
  marks: [
    Plot.lineY(long_excluded_df_heuristic_filtered, {
      x: "fixed_time_horizon",
      y: "excluded_count",
      stroke: "state",
      curve: "step-after"
    }),
    Plot.dot(long_excluded_df_heuristic_filtered, {
      x: "fixed_time_horizon",
      y: "excluded_count",
      fill: "state",
      r: 2.5,
