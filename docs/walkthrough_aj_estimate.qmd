...
### Debug excluded-adjusted

```{python}

# extract_aj_estimate_by_assumptions_polars(
#                 list_data_to_adjust_polars.get('full'),
#                 fixed_time_horizons=[1, 3, 5],
...
# exploded = assign_and_explode_polars(list_data_to_adjust_polars.get('full'), [1, 3, 5])
...
adjusted_data_list_polars = create_adjusted_data_list_polars(
  list_data_to_adjust=list_data_to_adjust_polars,
...
# type(adjusted_data_list_polars)

# adjusted_data_list_polars[0].schema
# adjusted_data_list_polars[1].schema
# adjusted_data_list_polars[2].schema
# adjusted_data_list_polars[3].schema
# adjusted_data_list_polars[4].schema
...
final_adjusted_data_polars = pl.concat(adjusted_data_list_polars, how="vertical")
...
print(aj_data_combinations.schema)
print(final_adjusted_data_polars.schema)


combined_adjusted_data = aj_data_combinations.join(
    final_adjusted_data_polars,
    on=["reference_group", "fixed_time_horizon", "censoring_assumption", "competing_assumption", "reals", "strata"],
    how="left"
)

print(combined_adjusted_data)


print(aj_data_combinations.select("strata").unique().sort("strata"))
print(final_adjusted_data_polars.select("strata").unique().sort("strata"))


aj_data_combinations.select(pl.col("strata"), pl.col("reference_group"), pl.col("fixed_time_horizon")).join(
  final_adjusted_data_polars.select(pl.col("strata"), pl.col("reference_group"), pl.col("fixed_time_horizon")),
  on=["reference_group", "strata", "fixed_time_horizon"]
)
...
ojs_define(data = combined_adjusted_data.to_pandas())
