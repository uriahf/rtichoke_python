...
def _visible_trace(fig, name):
    matches = [
        trace for trace in fig.data if trace.name == name and trace.visible is not False
    ]
...
def _y_at_x(trace, x):
    matches = [
        float(y) for tx, y in zip(trace.x, trace.y) if float(tx) == pytest.approx(x)
    ]
    assert len(matches) == 1
    return matches[0]
...
def test_time_gains_reference_is_population_and_horizon_specific():
    ...
    assert _y_at_x(traces[("perfect_model_population_a", True)], 0.01) == pytest.approx(
        0.06
    )
    assert _y_at_x(traces[("perfect_model_population_b", True)], 0.01) == pytest.approx(
        0.04
    )
    assert _y_at_x(
        traces[("perfect_model_population_a", False)], 0.01
    ) == pytest.approx(0.03)
    assert _y_at_x(
        traces[("perfect_model_population_b", False)], 0.01
    ) == pytest.approx(0.02)
