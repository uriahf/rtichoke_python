...
def test_static_one_model_one_population_and_metric_ids():
    ...
    assert spec["evaluations"] == [
        {"id": "evaluation-1", "model": "Model A", "population": "__shared_population__"}
    ]
    assert [metric["id"] for metric in spec["metrics"]] == [
        "true_positives", "true_negatives", "false_positives", "false_negatives",
        "sensitivity", "specificity", "false_positive_rate", "ppv", "npv",
        "lift", "predicted_positives", "ppcr", "net_benefit",
...
def test_zero_is_preserved_and_missing_metric_is_null():
    ...
    values = {value["metricId"]: value["estimate"] for value in spec["rows"][0]["values"]}
...
def test_time_table_maps_horizon_and_heuristic_context():
    ...
    data = prepare_performance_data_times(
        probs, reals, times, fixed_time_horizons=[5.0, 10.0],
        heuristics_sets=heuristics, by=0.5,
    )
    ...
    assert {tuple(row["context"].values()) for row in spec["rows"]} == {
        ("adjusted", "adjusted_as_negative"), ("excluded", "excluded")
    }
