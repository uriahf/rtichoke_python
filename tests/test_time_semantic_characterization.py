...
def test_multiple_models_keep_labels_and_colors_across_horizons(creator):
    fig = creator(
    ...
    for model, color in zip(("Model A", "Model B"), COLORS):
        traces = [trace for trace in fig.data if trace.name == model]
        assert len(traces) == len(HORIZONS)
        assert {trace.line.color for trace in traces} == {color}


def test_interventions_avoided_keeps_labels_and_colors_across_horizons():
...
def test_multiple_models_share_reference_context_at_each_horizon():
    ...
    for curve, names in expected.items():
...
def test_different_prevalence_populations_are_distinct_series_and_references():
    probs = {"Population low": PROBS_A, "Population high": PROBS_B}
    reals = {
        "Population low": np.array([1, 0, 0, 0, 0, 0]),
        "Population high": np.array([1, 1, 1, 0, 0, 0]),
    }
    times = {
        "Population low": np.array([3.0, 12.0, 13.0, 14.0, 15.0, 16.0]),
        "Population high": np.array([2.0, 7.0, 9.0, 12.0, 13.0, 14.0]),
    }
    ...
    assert _series_keys(performance_data) == {
        ...
        ("Population high", 5.0),
        ("Population low", 10.0),
        ("Population high", 10.0),
    }

    pr_names = _reference_names_by_horizon(
        _references(performance_data, "precision recall")
    )
    assert pr_names == {
        5.0: {"random_guess_Population low", "random_guess_Population high"},
        10.0: {"random_guess_Population low", "random_guess_Population high"},
    }
...
def test_equal_prevalence_populations_remain_series_but_references_collapse():
    ...
    assert pr_names == {5.0: {"random_guess"}, 10.0: {"random_guess"}}
...
def test_paired_model_population_inputs_are_generic_series_per_horizon():
    ...
    times = {
        ...
        pair_names[1]: np.array([2.0, 7.0, 9.0, 12.0, 13.0, 14.0]),
    }
    performance_data = _performance(probs, reals, times)
...
def test_time_calibration_has_one_identity_line_per_horizon_and_group_series():
    ...
    for group, color in zip(("Population A", "Population B"), COLORS):
        ...
        assert len(traces) == 2 * len(HORIZONS)
...
def test_time_performance_data_preserves_group_and_horizon_dimensions():
    ...
    assert {"reference_group", "fixed_time_horizon"}.issubset(
