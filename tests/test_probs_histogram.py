...
def test_authoritative_v0221_schema_validation_probability_threshold_and_ppcr():
    schema = json.loads(_SCHEMA_PATH.read_text(encoding="utf-8"))
    validator = Draft202012Validator(schema)

    # Confirm all 8 v2 chart type branches are represented in the authoritative schema
    chart_types: list[str] = []
    for item in schema.get("anyOf", []):

        def search(obj: object) -> None:
            if isinstance(obj, dict):
                if "properties" in obj and "type" in obj["properties"]:
                    tc = obj["properties"]["type"].get("const")
                    if tc and tc not in ("identity", "horizontal", "vertical", "path"):
                        chart_types.append(str(tc))
                for v in obj.values():
                    search(v)
    ...
    assert set(chart_types) == expected_types
    ...
    # 3. Negative schema validation: missing required property 'bins'
    bad_spec_missing_bins = dict(chart_thresh.spec)
    del bad_spec_missing_bins["bins"]
    assert not validator.is_valid(bad_spec_missing_bins)

    # 4. Negative schema validation: unsupported chart type
    bad_spec_unsupported_type = dict(chart_thresh.spec, type="unsupported_type")
    assert not validator.is_valid(bad_spec_unsupported_type)
