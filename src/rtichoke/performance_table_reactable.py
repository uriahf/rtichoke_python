...
def _bar_style(value: float | None, maximum: float = 1.0, color: str = "lightgreen") -> dict[str, str]:
...
def render_performance_table_reactable(
    ...
    display_columns = [
        "reference_group", "fixed_time_horizon", "censoring_heuristic",
        "competing_heuristic", "chosen_cutoff", "sensitivity", "specificity",
        "ppv", "npv", "lift", "predicted_positives", "net_benefit", "ppcr",
        "true_positives", "true_negatives", "false_positives", "false_negatives",
    ]
    data = performance_data.select([c for c in display_columns if c in performance_data.columns])
    rename_map = {
    ...
    models = data.get_column("Model").unique(maintain_order=True).to_list() if "Model" in data.columns else []
    colors = {model: color_values[i % len(color_values)] for i, model in enumerate(models)}
    ...
    def confusion_matrix(info: RowInfo):
        ...
        def matrix_style(colors: tuple[str, str, str]):
            return lambda info: _bar_style(info.value, float(total), colors[info.row_index])
    ...
    if "Time" in data.columns:
        columns.append(Column(id="Time", name="Time Horizon", format=ColFormat(digits=2), min_width=100))
    ...
    if "Competing Event" in data.columns:
        columns.append(Column(id="Competing Event", name="Competing Event", min_width=140))
    if stratified_by == "probability_threshold":
        columns.append(Column(id="Threshold", name="Probability Threshold", format=ColFormat(digits=2), min_width=130))
    columns.extend([
        Column(id="ppcr", name="Predicted Positives", cell=ppcr_cell, min_width=150,
        ...
        Column(id="net_benefit", name="Net Benefit", format=ColFormat(digits=2),
