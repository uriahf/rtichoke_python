...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      .rt-perf-wrap{overflow:auto;border:1px solid #e5e5e5;border-radius:3px;max-height:620px;background:#fff}
      ...
      .rt-perf thead th{position:sticky;top:0;z-index:3;background:#fff;font-weight:600;color:#333}
  ...
  function confusionMatrix(r) {
    ...
    return `<table class="rt-conf"><thead><tr><th></th><th>Real Positive</th><th>Real Negative</th><th></th></tr></thead><tbody>${rows.map((q,i)=>`<tr><td class="outcome">${q[0]}</td><td style="background:${metricBackground(q[1],total,q[3])}">${value(q[1])}</td><td style="background:${metricBackground(q[2],total,q[4])}">${value(q[2])}</td><td style="background:${metricBackground(q[1]+q[2],total,'lightgrey')}">${value(q[1]+q[2])}</td></tr>`).join("")}</tbody></table>`;
  }
  ...
  function render(rows, selector, isPpcr) {
    ...
    sorted.forEach(r=>{
      ...
      const td=document.createElement("td"); td.colSpan=isPpcr?9:11; td.innerHTML=confusionMatrix(r); detail.appendChild(td);
