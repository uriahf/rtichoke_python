 ...
 * The R summary-report template explicitly renders these figures at size=500.
 */
function drawRtichokeCurve(s, sel) {
  ...
  const W=500,H=500,m={top:35,right:30,bottom:65,left:65};
  ...
  const isDecision=String(s.title||"").toLowerCase().includes("decision")||String(s.y_label||"").toLowerCase()==="net benefit",isInterventions=String(s.y_label||"").toLowerCase().includes("interventions avoided");
  const strategyColor=g=>{const k=String(g||"").toLowerCase();if(k==="treat_none")return "#808080";return s.colors[g]||"#BEBEBE"};
  const traces=[];
  d3.group(s.references,d=>String(d.reference_group)).forEach((a,g)=>{const color=strategyColor(g);svg.append("path").datum(a).attr("fill","none").attr("stroke",color).attr("stroke-width",2).attr("d",line);traces.push(...a.map(d=>({...d,_color:color})))});
  ...
  svg.append("rect").attr("x",m.left).attr("y",m.top).attr("width",W-m.left-m.right).attr("height",H-m.top-m.bottom).attr("fill","transparent").on("mousemove",ev=>{const[mx,my]=d3.pointer(ev);let best=null,dist=Infinity;traces.forEach(d=>{if(!isFinite(+d.x)||!isFinite(+d.y))return;const dd=(x(+d.x)-mx)**2+(y(+d.y)-my)**2;if(dd<dist){dist=dd;best=d}});if(best&&dist<625)show(ev,best);else hide()}).on("mouseleave",hide);
}
