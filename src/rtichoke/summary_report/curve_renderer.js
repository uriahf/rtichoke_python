 ...
 * 550px-high widget with no internal title. ROC/Lift/etc. are tab headings.
 */
function drawRtichokeCurve(s, sel, strat) {
  const card=d3.select(sel); card.selectAll("*").remove();
  card.style("width","500px").style("max-width","100%").style("margin-left","0").style("margin-right","0");
  const W=500,H=550,m={top:25,right:10,bottom:40,left:60};
  const svg=card.append("svg").style("width","500px").style("max-width","100%").style("margin","0").attr("viewBox",`0 0 ${W} ${H}`),x=d3.scaleLinear().domain(s.x_range).range([m.left,W-m.right]),y=d3.scaleLinear().domain(s.y_range).range([H-m.bottom,m.top]);
  ...
  svg.append("text").attr("class","axis-label").attr("x",(m.left+W-m.right)/2).attr("y",H-8).attr("text-anchor","middle").text(s.x_label);
  ...
  if(strata.length>1){
    const wrap=card.append("div").attr("class","slider-wrap");
    const label=wrap.append("div").attr("class","slider-label");
    const prefix=strat==="ppcr"?"Predicted Positives (Rate):":"Probability Threshold:";
    const input=wrap.append("input").attr("type","range").attr("min",strata[0]).attr("max",strata[strata.length-1]).attr("step",Math.max(1e-6,...strata.slice(1).map((v,i)=>v-strata[i]).filter(v=>v>0).slice(0,1))).node();
